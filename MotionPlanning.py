...
if __name__ == "__main__":
    ...
    # robot.setAccelerationLimits([1000 for _ in range(robot.numLinks())])
    # robot.setVelocity([1000 for _ in range(robot.numLinks())])
    # robot.setVelocityLimits([1000 for _ in range(robot.numLinks())])
    # robot.setTorqueLimits([1000 for _ in range(robot.numLinks())])
    ...
    space = robotplanning.make_space(world,robot,edgeCheckResolution= 0.01)
    # space = robotcspace.RobotCSpace(robot,collide.WorldCollider(world))
    start = [0.0, 0.6399999999999998, 0.5399999999999999, -0.6200000000000001, 0.0, -1.5707963267948966, 0.0, 0.0]
    goal = [0.0, -0.5, 0.6000000000000001, -0.18000000000000002, 0.0, -1.5707963267948966, 0.0, 0.0]
